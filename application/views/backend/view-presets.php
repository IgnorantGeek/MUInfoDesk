...
<head>
    <meta charset="UTF-8">
    <title>Database View Presets</title>
...
<body>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-hover">
            ...
            <?php
            if($fetch_presets->num_rows() > 0)
            {
                foreach($fetch_presets->result() as $row)
                {
            ?>
                <tr>
                    <td><?php echo $row->presetID; ?></td>
                    <td><?php echo $row->description; ?></td>
                    <td><a href="#" class="delete_data_presets" id="<?php echo $row->presetID; ?>">Delete</td>
                </tr>
            <?php
                }
            }
    ...
    <script>
        $(document).ready(function(){
            $('.delete_data_presets').click(function(){
                var id = $(this).attr("id");
                if(confirm("Are you sure you want to delete this?"))
                {
                    window.location="<?php echo base_url();?>Databaseview/delete_data_presets/"+id;
                    return true;
                }
                else
                {
